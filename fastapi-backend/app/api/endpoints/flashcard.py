...
def get_flashcard_sets_by_subject(subject_id: int, db: Session = Depends(get_db)):
    """
    Get flashcard sets by subject ID (via document_id -> document -> subject).
    """
    flashcard_sets = (
        db.query(FlashcardSet)
        .join(Document, FlashcardSet.document_id == Document.id)
        .filter(Document.subject_id == subject_id)
        .all()
    )
    result = []
    for fset in flashcard_sets:
        flashcard_count = db.query(Flashcard).filter(Flashcard.set_id == fset.id).count()
        result.append({
            "flashcard_set_id": fset.id,
            "title": fset.title,
...
def get_flashcard_set_by_id(id: int, db: Session = Depends(get_db)):
    ...
    flashcards = db.query(Flashcard).filter(Flashcard.set_id == fset.id).all()
    
    # Map to expected response schema
    items = [FlashcardItem(front=f.term, back=f.definition) for f in flashcards]
