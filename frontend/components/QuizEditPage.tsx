...
import { getQuiz, updateQuiz, uploadQuizImage } from '@/lib/quizzes';
import {
  ArrowLeft, Plus, Trash2, Image as ImageIcon, X, Globe, Lock,
  Copy, Loader2, ChevronDown, Check, Circle,
  Square, AlignLeft, List, ChevronUp,
  type LucideIcon,
} from 'lucide-react';
...
export default function EditQuizPage() {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (res.success && res.data) {
          if (!res.data.isOwner) {
            setError('Bạn không có quyền chỉnh sửa quiz này');
            setLoadingData(false);
            return;
          }
          ...
          setQuestions(
            (res.data.questions || []).map((q: any) => ({
              ...
              imagePreview: q.image_url ? `${API_BASE}${q.image_url}` : '',
  ...
  const handleSubmit = async () => {
    ...
    setSaving(true);
    try {
      const res = await updateQuiz(token, quizId, {
        title: title.trim(),
      ...
      if (res.success) {
        ...
        setError(res.message || 'Lỗi cập nhật quiz');
      }
    } catch {
      setError('Lỗi kết nối máy chủ');
    }
      ...
      <div className="min-h-screen bg-gray-50">
        <nav className="bg-white shadow-sm border-b">
          <div className="max-w-3xl mx-auto px-4 py-3 flex items-center gap-3">
            ...
            <h1 className="text-lg font-bold text-gray-800">Chỉnh sửa Quiz</h1>
          </div>
        </nav>
