        ...
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            {editing ? (
              <div className="space-y-2">
                ...
                <div className="flex gap-2">
                  ...
                  <button onClick={() => { setEditing(false); setEditContent(comment.content); }} className="px-3 py-1.5 bg-slate-100 text-slate-600 text-xs font-semibold rounded-lg hover:bg-slate-200 transition">
                    Hủy
                  </button>
                </div>
              </div>
            ...
            <div className="flex items-center gap-3 mt-2 opacity-0 group-hover:opacity-100 transition-opacity">
              {canReply && token && (
                <button onClick={() => setReplying(!replying)} className="flex items-center gap-1 text-xs text-slate-500 hover:text-cyan-600 transition font-medium">
                  <Reply className="w-3.5 h-3.5" /> Trả lời
                </button>
              )}
              {isOwner && !editing && (
                <>
                  <button onClick={() => setEditing(true)} className="flex items-center gap-1 text-xs text-slate-500 hover:text-cyan-600 transition font-medium">
                    <Edit3 className="w-3.5 h-3.5" /> Sửa
                  </button>
                  <button onClick={handleDelete} className="flex items-center gap-1 text-xs text-slate-500 hover:text-rose-600 transition font-medium">
                    <Trash2 className="w-3.5 h-3.5" /> Xóa
                  </button>
                </>
              )}
              {localComment.children.length > 0 && (
                <button onClick={() => setCollapsed(!collapsed)} className="flex items-center gap-1 text-xs text-slate-400 hover:text-slate-600 transition font-medium ml-auto">
            ...
            {replying && (
              <div className="mt-3 space-y-2">
                ...
                <div className="flex gap-2">
                  ...
                  <button onClick={() => { setReplying(false); setReplyContent(''); }} className="px-3 py-1.5 bg-slate-100 text-slate-600 text-xs font-semibold rounded-lg hover:bg-slate-200 transition">
                    Hủy
                  </button>
                </div>
              </div>
  ...
  return (
    <div className="min-h-screen bg-slate-50 py-8 px-4 relative overflow-hidden">
      ...
      <div className="mx-auto max-w-3xl space-y-5 relative z-10">
        ...
        {/* Back */}
        <Link href="/forum" className="inline-flex items-center gap-2 px-5 py-2.5 text-sm font-medium text-slate-700 hover:text-slate-900 bg-white hover:bg-slate-100 rounded-xl border border-slate-200 transition-all">
          ...
          <div className="p-6 md:p-8">
            <div className="flex gap-4">
              ...
              <div className="flex-1 min-w-0">
                ...
                {/* Meta */}
                <div className="flex flex-wrap items-center gap-3 mb-5 text-xs text-slate-500">
                  <div className="flex items-center gap-1.5">
                  ...
                  <div className="flex flex-wrap gap-1.5 mb-5">
                    {post.tags.map(tag => (
                      <Link key={tag} href={`/forum?tag=${encodeURIComponent(tag)}`}
                        className="text-xs px-2.5 py-1 rounded-full bg-cyan-50 border border-cyan-200 text-cyan-700 font-medium hover:bg-cyan-100 transition">
                        #{tag}
                      </Link>
                    ))}
                  </div>
                ...
                {isOwner && (
                  <div className="flex items-center gap-3 mt-6 pt-5 border-t border-slate-100">
                    <Link
                      ...
                      className="flex items-center gap-1.5 px-4 py-2 bg-slate-50 border border-slate-200 text-slate-600 rounded-xl text-sm font-semibold hover:bg-cyan-50 hover:border-cyan-300 hover:text-cyan-700 transition"
                    >
                      <Edit3 className="w-4 h-4" /> Chỉnh sửa
                    </Link>
                    <button
                      onClick={handleDeletePost}
                      className="flex items-center gap-1.5 px-4 py-2 bg-slate-50 border border-slate-200 text-slate-600 rounded-xl text-sm font-semibold hover:bg-rose-50 hover:border-rose-300 hover:text-rose-600 transition"
                    >
        ...
        <div className="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden">
          <div className="px-6 py-4 border-b border-slate-100 flex items-center gap-2">
            ...
            <h2 className="font-bold text-slate-800">{post.comment_count} Bình luận</h2>
          </div>
            ...
            {token ? (
              <form onSubmit={handleAddComment} className="space-y-3">
                <textarea
                  ...
                  rows={4}
                  className="w-full px-4 py-3 border border-slate-200 rounded-xl text-sm font-mono focus:outline-none focus:ring-2 focus:ring-cyan-500 resize-y bg-slate-50"
                />
              ...
              <div className="text-center py-4 bg-slate-50 rounded-xl border border-slate-200 text-sm text-slate-500">
                <Link href="/login" className="text-cyan-600 font-semibold hover:underline">Đăng nhập</Link> để bình luận
              </div>
            )}
