...
const getIcon = (type: string) => {
  switch (type) {
    ...
    case 'exercise':
      return <FileCode className="w-7 h-7 text-emerald-400" />;
    case 'exam':
      return <FileText className="w-7 h-7 text-rose-400" />;
    default:
    ...
    <div className="min-h-screen bg-[#0B0F19] py-10 px-4 relative overflow-hidden">
      ...
      <div className="mx-auto max-w-7xl space-y-8 relative z-10">
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div>
            <p className="text-sm uppercase tracking-[0.24em] text-indigo-400 font-semibold">Tài liệu</p>
            <h1 className="mt-3 text-3xl font-bold tracking-tight text-white">Tài liệu nổi bật</h1>
            <p className="mt-2 text-sm leading-6 text-slate-400">Khám phá nội dung mới nhất và tải lên tài liệu của bạn ngay.</p>
          </div>
        ...
        <section className="rounded-3xl border border-white/10 bg-white/5 backdrop-blur-xl p-8 shadow-2xl transition-all hover:bg-white/[0.07]">
          <div className="flex items-center justify-between gap-4 mb-8 border-b border-white/10 pb-5">
            <div>
              <h2 className="text-xl font-bold text-white">Danh sách tài liệu</h2>
              <p className="mt-1 text-sm text-slate-400">Những tài liệu mới cập nhật và đáng chú ý.</p>
            </div>
                ...
                >
                  ...
                  <div className="flex items-start gap-4 relative z-10">
                    ...
                    <div className="min-w-0 flex-1">
                      <p className="text-xs font-semibold text-indigo-400 uppercase tracking-wider">{getTypeLabel(doc.doc_type)}</p>
                      <h3 className="mt-1 text-lg font-bold text-white line-clamp-2 leading-tight group-hover:text-indigo-300 transition-colors">{doc.file_name}</h3>
                    </div>
                  </div>
                  <div className="mt-6 flex items-center justify-between text-sm text-slate-400 relative z-10">
                    <span className="font-medium bg-white/5 px-3 py-1 rounded-lg border border-white/5">{formatBytes(doc.file_size)}</span>
                    <span className="rounded-lg bg-indigo-500/10 border border-indigo-500/20 px-3 py-1 text-indigo-300 font-mono shadow-sm">#{doc.subject_id}</span>
                  </div>
                </Link>
              ))}
            </div>
