...
export default function SubjectDetailComponent() {
    ...
    const fetchData = async () => {
      ...
      try {
        const [subjectRes, docsRes] = await Promise.all([
          ...
          fetch(`http://localhost:8000/api/v1/document/subject/${subjectId}`)
        ]);
        ...
        if (docsRes.ok) {
          ...
          setFeaturedDocs(docsData.documents || docsData || []);
        }
      } catch (err: any) {
  ...
  const handleGenerateQuiz = async () => {
    ...
    try {
      const response = await fetch(`http://localhost:8000/api/v1/ai/generate/${selectedDocs[0]}`, {
        method: 'POST',
        headers: {
